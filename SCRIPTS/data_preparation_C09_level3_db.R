...
#        and creating dataset only for C09 exposure!
#        - creating groups with: only C09A or C09B, and only C09C or C09D
#
...
## GROUP C09 BY ATC LEVEL 3 ----
prescriptions_with_level3_exposed <- prescriptions %>%
	select(indiv_ID, prescr_year, prescr_mnth, ATC_code) %>%
	filter(ATC_code %like% "C09%") %>%
	collect()
prescriptions_with_level3_exposed <- prescriptions_with_level3_exposed %>%
	mutate(ATC_level3 = substr(ATC_code, 1, 4)) %>%
	# group into: C09A, C09B - ACEIs; C09C, C09D - ARBs
	mutate(C09_group = case_when(
		ATC_level3 %in% ACEIs ~ "ACEI",
		ATC_level3 %in% ARBs ~ "ARB",
		TRUE ~ NA_character_
	)) %>%
	arrange(indiv_ID, C09_group, prescr_year, prescr_mnth) %>%
	mutate(prescr_date = ymd(
		paste(prescr_year, prescr_mnth, "15", sep = "-")
	)
	) %>%
	group_by(indiv_ID, C09_group, prescr_date) %>%
...
prescriptions_with_level3_exposed
setDT(prescriptions_with_level3_exposed)

prescriptions_with_level3_exposed[
	, start_exposure_date := prescr_date
]
...
retain_C09_group <- PD_patients_per_drug_group[
	N >= 5, C09_group
]
retain_C09_group <- str_subset(retain_C09_group, "N04", negate = TRUE)

## RESTRUCTURE DATASET: now, only one row per individual ------
data_restr <- dcast(
	dataset_long,
	indiv_ID + park_yn + first_N04_date ~ C09_group,
	value.var = c("med_time")
)
dim(data_restr)
names(data_restr)

data_restr <- data_restr[, -"NA"]
...
saveRDS(
	...
	file = here(
		"DATA",
		paste0("dataset_ready_for_analysis_exposure", prescriptions_exposure,
					 "_level3_C09.rds")
	)
)
