...
```{r read_data}
women.results <- read_csv("cur_results_women.csv")
men.results <- read_csv("cur_results_men.csv")
```
...
```{r sort_by_atc}
cur.results <- bind_rows(
	women.results %>%
		tibble::add_column(sex = "women"),
	men.results %>%
		tibble::add_column(sex = "men")
)
unique_ATC_codes <- cur.results %>%
	distinct(ATC_code) %>%
	pull()
```


```{r plot_plotly,eval=FALSE,include=FALSE,echo=FALSE,warning=FALSE,out.height=1200,fig.height=1200}
# create two columns: one for selection, the other for plot
shared.women <- SharedData$new(cur.results)

bscols(widths = c(3, NA, NA),
	list(
		filter_checkbox(id = "p-val_exclude", label = "(Unadjusted) P-value filtering",
	...
	# plot for women
	plot_ly(data = shared.atc %>%
						filter(sex == "women"),
					x = ~HR,
					y = ~ATC_code,
					text = ~hover.text,
					error_x = list(
					 		array = ~HR_u, arrayminus = ~HR_l,
					 		color = ~as.factor(group), symmetric = FALSE),
					height = 1200) %>%
		add_markers(color = ~as.factor(group)) %>%
		layout(xaxis = list(range = c(0,10),
													mirror = "ticks"),
					yaxis = list(title = list(text = "ATC code")),
					title = "WOMEN"),
	# plot for men
	plot_ly(data = shared.atc %>%
						filter(sex == "men"),
...
bscols(widths = c(3, NA),
	...
	ggplotly(plot.sort.strata, height = nrow(cur.results) * 10,
						tooltip = c("y", "text"))
)
...
hr_order_women <- cur.results %>%
		filter(sex == "women") %>%
		distinct(ATC_code, HR) %>%
		arrange(HR)
hr_order_men <- cur.results %>%
		filter(sex == "men") %>%
		distinct(ATC_code, HR) %>%
		arrange(HR)
...
cur.results$ATC_code <- factor(
	cur.results$ATC_code,
	levels = hr_order,
	ordered = TRUE
)
```
...
# if there are various ATC codes for women and men, I need to merge those
pv_order_women <- cur.results %>%
		filter(sex == "women") %>%
		distinct(ATC_code, p.adj.FDR) %>%
		arrange(p.adj.FDR)
pv_order_men <- cur.results %>%
		filter(sex == "men") %>%
		distinct(ATC_code, p.adj.FDR) %>%
		arrange(p.adj.FDR)
...
```{r plot_diff_ggplotly,include=TRUE,echo=FALSE,warning=FALSE,out.height='100%'}
clrs <- list(men = "#bad744", women = "#5b8124")

# create label on the top result
label.df <- cur.results %>%
	# filter(ATC_code == "G03")
...
# range of the X axis
range.x <- range(
	range(cur.results$HR_women, na.rm = TRUE),
	range(cur.results$HR_men, na.rm = TRUE)
)

plot_sex_diff <- ggplot(cur.results,
  ...
  labs(
	...
	coord_cartesian(
		ylim = c(-0.5, max(as.numeric(cur.results$ATC_code)) + 0.5),
